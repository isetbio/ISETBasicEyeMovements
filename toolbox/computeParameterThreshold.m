function [paramValueThreshold, questObj, psychometricFunction, fittedPsychometricParams]  = computeParameterThreshold(...
    ...
    thresholdPara, questEnginePara, meanLuminance, varargin)
    ...
    while (nextFlag)
        ...
        if (true)
            ...
            for oo = 1:length(theSceneEngines)
                ...
                % adjust luminance level based on input
%                 bitDepth = sceneGet(theTestSceneSequences{testedIndex}{oo}, 'bitDepth');
%                 if isempty(bitDepth), error('Compression parameters required'); end
%                 theTestSceneSequences{testedIndex}{oo}{1,1}.data.bitDepth = 32;
                theTestSceneSequences{testedIndex}{oo}{1,1} = sceneAdjustLuminance(theTestSceneSequences{testedIndex}{oo}{1,1}, meanLuminance);
            end
            
%             visualizeScene(theTestSceneSequences{testedIndex}{1}{1,1}, ...
