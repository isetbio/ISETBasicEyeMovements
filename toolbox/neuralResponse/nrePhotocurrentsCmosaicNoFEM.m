...
%}
    ...
    if (isempty(neuralEngineOBJ.neuralPipeline))
        ...
        theConeMosaic = cMosaic(...
            ...
            'eccentricityDegs', neuralResponseParamsStruct.coneMosaicParams.eccentricityDegs, ...
            'integrationTime', neuralResponseParamsStruct.coneMosaicParams.integrationTime ...
            );
        
        % Generate optics appropriate for the mosaic's eccentricity
        oiEnsemble = theConeMosaic.oiEnsembleGenerate(neuralResponseParamsStruct.coneMosaicParams.eccentricityDegs, ...
    ...
    for idx = 1:length(noiseFlags)
        if (contains(ieParamFormat(noiseFlags{idx}), 'none'))
            ...
            theNeuralResponses(noiseFlags{idx}) = computePhotocurrent(...
                noiseFreeConeExcitationResponses, temporalSupportSeconds, theConeMosaic.noiseFlag); % , neuralResponseParamsStruct.eyeMovementsParams.keptResponsesDurationSeconds
...
function impulseResponse = computePhotocurrentImpulseResponse(timeAxis)
    ...
    % in @cMosaic
    load('/Users/qiyuanfeng/Documents/MATLAB/toolboxes/ISETBioCSFGenerator/osLinearFilters25CDM2.mat', 'osLinearFilters', 'dt');
    t = (1:size(osLinearFilters,1))*dt;
    
    impulseResponse = 2*interp1(t, squeeze(osLinearFilters(:,1)), t); % keep all time point
